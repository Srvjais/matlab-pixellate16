...
plantmp=imcrop(inImg,[bboxplan(1), bboxplan(2), bboxplan(1)+bboxplan(3), bboxplan(2)+bboxplan(4)]);
imtool(plantmp);

plan(:,:)=   filtr(plantmp,Red);
plan=bwareaopen(plan,500);
planRedProps=regionprops(plan);         %clears Noise,gets Props
imtool(plan)

plan(:,:)=   filtr(plantmp,Green);   
plan=bwareaopen(plan,500);
planGreenProps=regionprops(plan);       %clears Noise,gets Props
imtool(plan)
...
for i=1:r                                                       %#ok<*SAGROW>
    j=i-1+findClosest(plansRed(i:end,:),[bboxplan(3),bboxplan(4)]);
    tmp=plansRed(j,:);
...
for i=1:g
    j=i-1+findClosest(plansGreen(i:end,:),[bboxplan(3),bboxplan(4)]);
    tmp=plansGreen(j,:);
